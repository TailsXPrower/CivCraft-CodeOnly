		...
		for (Town town : civ.getTowns()) {
			...
			if (civ.getResearchTech() != null) {
				...
				for (int i = 0; i < size; i++) {
					...
					if (i <= blockCount) {
						sb = new SimpleBlock(Material.GREEN_WOOL, Bukkit.createBlockData(Material.GREEN_WOOL));
						sb.x = bcoord.getX(); sb.y = bcoord.getY(); sb.z = bcoord.getZ();
						sb.worldname = bcoord.getWorldname();
						sbs.add(sb);
					} else {
						sb = new SimpleBlock(Material.BLACK_WOOL, Bukkit.createBlockData(Material.BLACK_WOOL));
						sb.x = bcoord.getX(); sb.y = bcoord.getY(); sb.z = bcoord.getZ();
						sb.worldname = bcoord.getWorldname();
						sbs.add(sb);				
					}
