...
import ru.tailsxcraft.civcraft.structure.wonders.GlobeTheatre;
import ru.tailsxcraft.civcraft.structure.wonders.GreatLibrary;
...
public class BuildAsyncTask extends CivAsyncTask {
	...
	private boolean start() {
		...
		if (buildable instanceof Wonder) {
			CivMessage.playSoundCiv(buildable.getCiv(), "wonder.completed");
			if ( buildable instanceof GreatLibrary ) {
				if (buildable.getCiv().getActiveGoldenAge() != null) {
					buildable.getCiv().getActiveGoldenAge().setLength(buildable.getCiv().getActiveGoldenAge().getLength()+8);
					CivMessage.sendCiv(buildable.getCiv(), CivColor.Gold+"Золотой век нашей цивилизации был продлён, так как было построено чудо света "+buildable.getDisplayName());
					buildable.onComplete();
					return false;
    			}
            	
            	GoldenAge goldenAge = new GoldenAge();
    			goldenAge.start(buildable.getCiv());
			} else if ( buildable instanceof NotreDame ) {
				if (buildable.getCiv().getActiveGoldenAge() != null) {
					buildable.getCiv().getActiveGoldenAge().setLength(buildable.getCiv().getActiveGoldenAge().getLength()+8);
					CivMessage.sendCiv(buildable.getCiv(), CivColor.Gold+"Золотой век нашей цивилизации был продлён, так как было построено чудо света "+buildable.getDisplayName());
					buildable.onComplete();
            	...
            	GoldenAge goldenAge = new GoldenAge();
    			goldenAge.start(buildable.getCiv());
			} else if ( buildable instanceof TheColossus ) {
				if (buildable.getCiv().getActiveGoldenAge() != null) {
					buildable.getCiv().getActiveGoldenAge().setLength(buildable.getCiv().getActiveGoldenAge().getLength()+8);
					CivMessage.sendCiv(buildable.getCiv(), CivColor.Gold+"Золотой век нашей цивилизации был продлён, так как было построено чудо света "+buildable.getDisplayName());
					buildable.onComplete();
            	...
            	GoldenAge goldenAge = new GoldenAge();
    			goldenAge.start(buildable.getCiv());
			} else if ( buildable instanceof GlobeTheatre ) {
				if (buildable.getCiv().getActiveGoldenAge() != null) {
					buildable.getCiv().getActiveGoldenAge().setLength(buildable.getCiv().getActiveGoldenAge().getLength()+8);
					CivMessage.sendCiv(buildable.getCiv(), CivColor.Gold+"Золотой век нашей цивилизации был продлён, так как было построено чудо света "+buildable.getDisplayName());
					buildable.onComplete();
			...
			CivMessage.globalSoundExceptCiv(buildable.getCiv(), "failed.wonder");
		} else {
			CivMessage.sendSoundTown(buildable.getTown(), "build.completed");
		}
		buildable.onComplete();
		return false;
	}
