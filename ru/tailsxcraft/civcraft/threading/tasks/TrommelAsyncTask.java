...
import ru.tailsxcraft.civcraft.structure.Trommel.Mineral;
	...
	public void processTrommelUpdate() {
		...
		for (int i = 0; i < trommel.skippedCounter+1; i++) {
			...
			for(ItemStack stack : contents) {
				...
				if (ItemManager.getType(stack) == CivData.COBBLESTONE) {
					try {
				...
				if (ItemManager.getType(stack) == CivData.STONE) {

					if (this.trommel.getLevel() >= 2 && ItemManager.getData(stack) == 
							ItemManager.getData(ItemManager.getMaterialData(CivData.STONE, CivData.GRANITE.getId()))) {
						try {
							this.updateInventory(Action.REMOVE, source_inv, ItemManager.createItemStack(CivData.STONE, 1));
						} catch (InterruptedException e) {
							return;
						}
						...
						if (rand1 < ((int)((trommel.getGraniteChance(Mineral.CRYSTAL))*randMax))) {
							...
							newItem = ItemManager.createItemStack(CivData.STONE, 1);
					...
					if (this.trommel.getLevel() >= 3 && ItemManager.getData(stack) == 
							ItemManager.getData(ItemManager.getMaterialData(CivData.STONE, CivData.DIORITE.getId()))) {
						try {
							this.updateInventory(Action.REMOVE, source_inv, ItemManager.createItemStack(CivData.STONE, 1));
						} catch (InterruptedException e) {
							return;
					...
					if (this.trommel.getLevel() >= 4 && ItemManager.getData(stack) == 
							ItemManager.getData(ItemManager.getMaterialData(CivData.STONE, CivData.ANDESITE.getId()))) {
						try {
							this.updateInventory(Action.REMOVE, source_inv, ItemManager.createItemStack(CivData.STONE, 1));
						} catch (InterruptedException e) {
							return;
