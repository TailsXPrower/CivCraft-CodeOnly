...
import ru.tailsxcraft.civcraft.items.components.ItemComponent;
...
//	public LinkedList<ItemStack> shaplessIngredientList = new LinkedList<ItemStack>();
	...
	public LoreCraftableMaterial(String id, Material item_id, short damage) {
		super(id, item_id, damage);
	}
	...
	//			key += "mc_"+stack.getTypeId()+"_"+stack.getDurability()+",";
				key += "mc_"+ItemManager.getType(stack)+",";
	...
	public static String getShapelessRecipeKey(ItemStack[] matrix) {
		...
		for (int i = 0; i < matrix.length; i++) {
			...
			//	item = "mc_"+stack.getTypeId()+"_"+stack.getDurability();
				item = "mc_"+ItemManager.getType(stack)+",";
			}
	...
	private void buildComponents() {
		...
		if (compInfoList != null) {
			for (HashMap<String, String> compInfo : compInfoList) {
				String className = "ru.tailsxcraft.civcraft.items.components."+compInfo.get("name");
				Class<?> someClass;
	...
	public static void buildRecipes() {
		...
		for (LoreCraftableMaterial loreMat : materials.values()) {
			...
			if (loreMat.isShaped()) {
				...
				ShapedRecipe recipe = new ShapedRecipe(new NamespacedKey(CivCraft.getPlugin(), loreMat.getId()), stack);
				...
				for (ConfigIngredient ingred : configMaterial.ingredients.values()) {
					...
					if (ingred.custom_id == null) {
						recipe.setIngredient(ingred.letter.charAt(0), ingred.type_id);
						ingredStack = ItemManager.createItemStack(ingred.type_id, 1);
					} else{
						...
						if (customMat.item_id != null) {
							recipe.setIngredient(ingred.letter.charAt(0), customMat.item_id);
						} else {
							CivLog.warning("Couldn't find custom material id:"+ingred.custom_id);
						}
				...
				/* Shapeless Recipe */						
				ShapelessRecipe recipe = new ShapelessRecipe(new NamespacedKey(CivCraft.getPlugin(), loreMat.getId()), stack);
				...
				for (ConfigIngredient ingred : configMaterial.ingredients.values()) {
					...
					try {
					if (ingred.custom_id == null) {
						recipe.addIngredient(ingred.count, ingred.type_id);
						ingredStack = ItemManager.createItemStack(ingred.type_id, 1);
					} else if (ingred.custom_id != null) {
						LoreCraftableMaterial customLoreMat = materials.get(ingred.custom_id);
						ConfigMaterial customMat = customLoreMat.configMaterial;
						if ( customMat.item_id != null ) 
						{
							recipe.addIngredient(ingred.count, customMat.item_id);
							ingredStack = LoreMaterial.spawn(customLoreMat);
						}
					}
	...
	public static ItemStack deserializeEnhancements(ItemStack stack, String serial) {
		...
		for (String enhString : enhancementsStrs) {
			...
			try {
				...
				// attrs.addEnhancement(new NamespacedKey(CivCraft.getPlugin(), cls.getSimpleName()), null, null);
