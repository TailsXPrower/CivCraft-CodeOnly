...
public class ConfigMaterial {
	...
	public static void loadConfig(FileConfiguration cfg, Map<String, ConfigMaterial> materials){
		...
		for (Map<?, ?> b : configMaterials) {
			...
			mat.item_id = Material.matchMaterial((String)b.get("item_id"));
			...
			if (category != null) {
				...
				if (mat.category.toLowerCase().contains("1")) {
					mat.tier = 1;
				} else if (mat.category.toLowerCase().contains("2")) {
					mat.tier = 2;
				} else if (mat.category.toLowerCase().contains("3")) {
					mat.tier = 3;
				} else if (mat.category.toLowerCase().contains("4")) {
					mat.tier = 4;
				} else {
					mat.tier = 0;
				}
			...
			if (configIngredients != null) {
				...
				for (Map<?, ?> ingred : configIngredients) {
					...
					ingredient.type_id = Material.matchMaterial((String)ingred.get("type_id"));
