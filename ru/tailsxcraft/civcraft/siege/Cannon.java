package ru.tailsxcraft.civcraft.siege;
	...
	private void processCommandSigns(Template tpl, BlockCoord corner) {
		for (BlockCoord relativeCoord : tpl.commandBlockRelativeLocations) {
			...
			case "/cannon":
				...
				Directional direct = (Directional)sb.getData();
				
				switch (direct.getFacing()) {
				case EAST:
				...
				signDirection = (Directional)sb.getData();
	...
	public void processFire(PlayerInteractEvent event) throws CivException {
		...
		if (this.tntLoaded < tntCost) {
			if (event.getAction().equals(Action.LEFT_CLICK_BLOCK)) {
				...
				if (stack != null) {
					if (ItemManager.getType(stack) == CivData.TNT) {
...
//			}
	...
	public void processAngle(PlayerInteractEvent event) throws CivException {
		...
		if (signDirection.getFacing() == BlockFace.EAST || signDirection.getFacing() == BlockFace.WEST) {
	...
	private void destroy() {
		for (BlockCoord b : blocks) {
			...
			if (b.getBlock().getType().equals(Material.COAL_BLOCK)) {
				ItemManager.setTypeIdAndData(b.getBlock(), CivData.GRAVEL, Bukkit.createBlockData(CivData.GRAVEL), false);
			} else {
				ItemManager.setTypeIdAndData(b.getBlock(), CivData.AIR, Bukkit.createBlockData(CivData.AIR), false);
			}
		}
		
		ItemManager.setTypeIdAndData(fireSignLocation.getBlock(), CivData.AIR, Bukkit.createBlockData(CivData.AIR), false);
		ItemManager.setTypeIdAndData(angleSignLocation.getBlock(), CivData.AIR, Bukkit.createBlockData(CivData.AIR), false);
		ItemManager.setTypeIdAndData(powerSignLocation.getBlock(), CivData.AIR, Bukkit.createBlockData(CivData.AIR), false);
