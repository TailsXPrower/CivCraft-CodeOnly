...
import ru.tailsxcraft.civcraft.endgame.EndConditionScience;
...
import ru.tailsxcraft.civcraft.structure.Capitol;
import ru.tailsxcraft.civcraft.structure.RespawnLocationHolder;
...
import ru.tailsxcraft.civcraft.threading.tasks.UpdateTechBar;
...
public class Civilization extends SQLObject {
	...
	private GoldenAge activeGoldenAge = null;
	private int timeGoldenAge = 0;
	...
	public static void init() throws SQLException {
		if (!SQL.hasTable(TABLE_NAME)) {
			String table_create = "CREATE TABLE " + SQL.tb_prefix + TABLE_NAME+" (" + 
					...
					"`timeGoldenAge` int(11) DEFAULT 0," +
...
//		Resident res = CivGlobal.getResidentViaUUID(UUID.fromString(resUUID));
		...
		this.timeGoldenAge = rs.getInt("timeGoldenAge");
		...
		for (ConfigTech tech : this.getTechs())
		{
			if (tech.era > this.getCurrentEra()) {
				CivMessage.playSoundCiv(this, "new.era");
	...
	public void saveNow() throws SQLException {
		...
		hashmap.put("timeGoldenAge", this.getTimeGoldenAge());
	...
	public void addTech(ConfigTech t) {
		...
		if (t.era > this.getCurrentEra()) {
			CivMessage.playSoundCiv(this, "new.era");
			if ( t.era == 3 ) {
				if (this.getActiveGoldenAge() != null) {
					this.getActiveGoldenAge().setLength(this.getActiveGoldenAge().getLength()+8);
					CivMessage.sendCiv(this, CivColor.Gold+"Золотой век нашей цивилизации был продлён, так как мы перешли в Эпоху Возрождения!");
    				return;
    			}
            	
            	GoldenAge goldenAge = new GoldenAge();
    			goldenAge.start(this);
			}
			this.setCurrentEra(t.era);
		} else {
			CivMessage.playSoundCiv(this, "research.completed");
		}
	}
...
//	}
	...
	public void addBeakers(double beakers) {
		...
		if (getResearchProgress() >= getResearchTech().getAdjustedBeakerCost(this)) {
			CivMessage.sendMessageCiv(this, CivColor.LightGreen+"Наша цивилизация изучила технологию "+CivColor.LightBlue+getResearchTech().name);
		...
		if ((percentageComplete % 10) == 0) {
			...
			if (percentageComplete != lastTechPercentage) {
				CivMessage.sendMessageCiv(this, CivColor.LightGreen+"Наша цивилизация изучила технологию "+CivColor.LightBlue+getResearchTech().name+CivColor.LightGreen+" на "+CivColor.Gold+percentageComplete+"%");
...
//				}
	...
	public int getHappiness() {
		int civHappinessRate = 0;
		for ( Town town : this.getTowns()) {
			civHappinessRate += town.getHappiness().total;
		}
		
		return civHappinessRate;
	}
	...
	public String getHappinessString() {
		...
		if ( this.getTimeGoldenAge() == 0 ) {
			if ( getHappiness() < 1500 ) return "("+getHappiness()+"/1500) счастья накоплено.";
		} else if ( this.getTimeGoldenAge() == 1 ) {
			if ( getHappiness() < 4500 ) return "("+getHappiness()+"/4500) счастья накоплено.";
		} else if ( this.getTimeGoldenAge() == 3 ) {
			if ( getHappiness() < 9000 ) return "("+getHappiness()+"/9000) счастья накоплено.";
		} else if ( this.getTimeGoldenAge() == 4 ) {
			if ( getHappiness() < 13500 ) return "("+getHappiness()+"/13500) счастья накоплено.";
		} else if ( this.getTimeGoldenAge() >= 5 ) {
			return "Максимальное количество счастья накоплено.";
		}
		
		return "(0/0) счастья накоплено.";
	}
	...
	public GoldenAge getActiveGoldenAge() {
		return activeGoldenAge;
	}

	public void setActiveGoldenAge(GoldenAge goldenAge) {
		// TODO Auto-generated method stub
		this.activeGoldenAge = goldenAge;
	}
	
	public int getTimeGoldenAge() {
		// TODO Auto-generated method stub
		return this.timeGoldenAge;
	}
	
	public void setTimeGoldenAge(int time) {
		// TODO Auto-generated method stub
		this.timeGoldenAge = time;
	}
