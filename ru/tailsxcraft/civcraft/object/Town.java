...
import ru.tailsxcraft.civcraft.components.AttributeBase;
import ru.tailsxcraft.civcraft.components.AttributeRate;
import ru.tailsxcraft.civcraft.components.AttributeWarUnhappiness;
import ru.tailsxcraft.civcraft.components.Component;
import ru.tailsxcraft.civcraft.config.CivSettings;
import ru.tailsxcraft.civcraft.config.ConfigBuff;
...
import ru.tailsxcraft.civcraft.interactive.InteractiveBuildableRefresh;
...
import ru.tailsxcraft.civcraft.structure.ResearchLab;
import ru.tailsxcraft.civcraft.structure.School;
...
import ru.tailsxcraft.civcraft.threading.sync.SyncUpdateTags;
...
import ru.tailsxcraft.civcraft.util.WorldCord;
...
//		}
	...
	public AttrSource getCultureRate() {
		...
		if (this.getCiv().getActiveGoldenAge() != null) {
			newRate = rate * 1.35;
			rates.put("Golden Age", newRate - rate);
			rate = newRate;
		}
	...
	public void addAccumulatedCulture(double generated) {
		...
		if (this.getCultureLevel() != CivSettings.getMaxCultureLevel()) {
			if (this.culture >= clc.amount) {
				...
				CivMessage.sendSoundTown(this, "borders.expand");
			}
		}
		return;
	}
	...
	public AttrSource getBeakers() {
		...
		for (Structure struct : this.structures.values()) {
			if (struct instanceof University)
			{
				...
				beakers += university.getBonusBeakers(); 
				universities += university.getBonusBeakers(); 
			}
			for (Component comp : struct.attachedComponents) {
				if (comp instanceof AttributeBase) {
					AttributeBase as = (AttributeBase)comp;
		...
		sources.put("University", universities);
