...
import ru.tailsxcraft.civcraft.command.civ.CivDiplomacyGiftCommand;
import ru.tailsxcraft.civcraft.command.civ.CivInfoCommand;
...
public class AdminCivCommand extends CommandBase {
	...
	public void init() {
		...
		commands.put("goldenage", "Управление Золотым веком");
		...
		commands.put("addtown", "Позволяет добавить фальшивый город к указанной цивилизации");
		commands.put("checkhappiness", "Показывает общее количество счастья в цивилизации");
	}
	...
	public void checkhappiness_cmd() throws CivException {
		Civilization сiv = getNamedCiv(1);
		
		int civHappinessRate = 0;
		for ( Town town : сiv.getTowns()) {
			civHappinessRate += town.getHappiness().total;
		}
		
		CivMessage.sendSuccess(sender, "Общее количество счастья в городах этой цивилизации: "+civHappinessRate);
	}
	...
	public void addtown_cmd() throws CivException {
		...
		String cityName = (new BigInteger(130, random).toString(32));
		Resident fake = null;
		try {
			fake = new Resident(UUID.randomUUID(), "RANDOM_"+name);
		} catch (InvalidNameException e) {
			//ignore
		}
		
		double diameter = 1000*2;
		Location center = new Location(((Player)sender).getWorld(), 0, 0, 0);//Center of the circle
		Random rand = new Random();
		double x = center.getX() + (rand.nextDouble()*diameter - 1000);
		double z = center.getY() + (rand.nextDouble()*diameter - 1000);
		Location spawn = new Location(((Player)sender).getWorld(), x, 80, z);
		
		Town town = Town.newTown(fake, cityName, civ, true, false, spawn);

		CivMessage.sendSuccess(sender, "Вы успешно создали новый город для указанной вами Цивилизации.");
	}
	...
	public void goldenage_cmd() throws CivException {
		AdminCivGoldenAgeCommand cmd = new AdminCivGoldenAgeCommand();	
		
		cmd.onCommand(sender, null, "goldenage", this.stripArgs(args, 1));	
	}
