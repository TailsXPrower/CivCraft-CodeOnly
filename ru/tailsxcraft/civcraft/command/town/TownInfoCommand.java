...
import ru.tailsxcraft.civcraft.config.ConfigHappinessState;
import ru.tailsxcraft.civcraft.config.ConfigTownLevel;
...
import ru.tailsxcraft.civcraft.object.AttrSource;
...
import ru.tailsxcraft.civcraft.structure.Cottage;
import ru.tailsxcraft.civcraft.structure.Mine;
import ru.tailsxcraft.civcraft.structure.Structure;
import ru.tailsxcraft.civcraft.structure.Temple;
import ru.tailsxcraft.civcraft.structure.TownHall;
import ru.tailsxcraft.civcraft.structure.University;
...
public class TownInfoCommand extends CommandBase {
	...
	public void init() {
		...
		commands.put("university", "Показывает информацию об Университете.");
...
//		CivMessage.send(sender, beakerSources.getRateDisplayString(CivColor.Green, CivColor.LightGreen));
	...
	public void buffs_cmd() throws CivException {
	    ...
	    if ( town.getCiv().getActiveGoldenAge() != null ) {
	    	out.add(CivColor.Gold+"Увеличена наука, продуктивность, урожайность, производство культуры на 35% благодаря Золотому Веку");
	    }
	...
	public void cottage_cmd() throws CivException {
		...
		for (Structure struct : town.getStructures()) {
			...
			if (!struct.isDestroyed()) {
				...
				out.add(CivColor.Green+"   "+CivSettings.CURRENCY_NAME+": "+CivColor.Yellow+coins+
	...
	public void university_cmd() throws CivException {
		Town town = getSelectedTown();
		ArrayList<String> out = new ArrayList<String>();	
		
		CivMessage.sendHeading(sender, town.getName()+" "+"Информация о Университете");
		double total = 0;
		...
		for (Structure struct : town.getStructures()) {
			...
			University university = (University)struct;
			...
			out.add(color+"Университет"+" ("+struct.getCorner()+")");
			out.add(CivColor.Green+"    "+CivSettings.localize.localizedString("Level")+" "+CivColor.Yellow+university.getLevel()+
					CivColor.Green+" "+CivSettings.localize.localizedString("count")+" "+CivColor.Yellow+"("+university.getCount()+"/"+university.getMaxCount()+")");
			out.add(CivColor.Green+"    "+"Общая прибавка:"+" "+CivColor.Yellow+university.getBonusBeakers());
			out.add(CivColor.Green+"    "+CivSettings.localize.localizedString("LastResult")+" "+CivColor.Yellow+university.getLastResult().name());
			
			total += university.getBonusBeakers(); //XXX estimate based on tile radius of 1.
			
		}
		out.add(CivColor.Green+"----------------------------");
		out.add(CivColor.Green+CivSettings.localize.localizedString("SubTotal")+" "+CivColor.Yellow+total);
		out.add(CivColor.Green+CivSettings.localize.localizedString("Total")+" "+CivColor.Yellow+df.format(total)+" "+"Пробирок.");
	...
	public static void show(CommandSender sender, Resident resident, Town town, Civilization civ, CommandBase parent) throws CivException {
		...
		if (resident == null || civ.hasResident(resident) || isAdmin) {
			...
			CivMessage.send(sender, CivColor.Green+CivSettings.localize.localizedString("cmd_town_growth")+" "+CivColor.LightGreen+Math.round(town.getGrowth().total)+" " +
									CivColor.Green+CivSettings.localize.localizedString("Hammers")+" "+CivColor.LightGreen+Math.round(town.getHammers().total)+" "+
									CivColor.Green+CivSettings.localize.localizedString("Beakers")+" "+CivColor.LightGreen+Math.round(town.getBeakers().total));
...
//					CivColor.LightGreen+df.format(town.getHappinessPercentage()*100)+"%");
		...
		if (isAdmin) {
			...
			CivMessage.send(sender, CivColor.Red+CivSettings.localize.localizedString("cmd_town_info_showWars")+" "+CivColor.White+wars);
