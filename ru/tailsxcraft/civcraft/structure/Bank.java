...
//	private static final int EMERALD_SIGN = 3;
	...
	private String getNonResidentFeeString() {
		return CivColor.Gold+CivSettings.localize.localizedString("bank_sign_fee")+" "+CivColor.White+((int)(this.nonMemberFeeComponent.getFeeRate()*100) + "%").toString();		
	}
	...
	public void exchange_for_coins(Resident resident, Material itemId, double coins) throws CivException {
		...
		int count = resident.takeItemsInHand(itemId, null);
		if (count == 0)
		{
	...
	public void processSignAction(Player player, StructureSign sign, PlayerInteractEvent event) {
		...
		case "sell":
			...
			Inventory bankInv = LoreGuiInventory.createGuiInventory(player, 18, "Банк");
			
			ItemStack bankInf = LoreGuiItem.build(CivColor.LightGreen+"Информация о Банке", Material.PAPER, new String[]{CivColor.Gold+"Уровень: "+CivColor.White+getTown().saved_bank_level,getNonResidentFeeString()});
			bankInv.setItem(8, bankInf);
			
			ItemStack sell = LoreGuiItem.build(CivColor.LightGreen+"Продать", Material.LIME_CONCRETE);
			sell = LoreGuiItem.setAction(sell, "SellItems");
			sell = LoreGuiItem.setActionData(sell, "townId", ""+this.getTown().getId());
			bankInv.setItem(17, sell);
	...
	public void onPostBuild(BlockCoord absCoord, SimpleBlock commandBlock) {
		...
		if (commandBlock.command.equals("/bank")) {
			...
			structSign.setText(CivColor.Green+"Банк"+"\n"+CivColor.Black+"Нажмите, чтобы"+"\n"+"продать");
