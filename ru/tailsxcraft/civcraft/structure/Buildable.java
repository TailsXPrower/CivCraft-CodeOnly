...
import ru.tailsxcraft.civcraft.interactive.InteractiveBuildCommand;
...
public abstract class Buildable extends SQLObject {
	...
	public void buildPlayerPreview(Player player, Location centerLoc, Template tpl) throws CivException, IOException {
		...
		if (!War.isWarTime() && CivSettings.showPreview && resident.getShowBuild() != "none")
		{
	...
	protected Location repositionCenter(Location center, String dir, double x_size, double z_size) throws CivException {
		...
		if (this.isTileImprovement()) {
			...
			if (dir.equalsIgnoreCase("east")) {
				loc.setZ(loc.getZ() - (z_size / 2));
				loc = center.getChunk().getBlock(0, center.getBlockY(), 0).getLocation();
				loc.setX(loc.getX() + SHIFT_OUT+16);
			}
			else if (dir.equalsIgnoreCase("west")) {
				loc.setZ(loc.getZ() - (z_size / 2));
				loc = center.getChunk().getBlock(0, center.getBlockY(), 0).getLocation();
				...
				loc.setX(loc.getX() - (x_size / 2));
				loc = center.getChunk().getBlock(0, center.getBlockY(), 0).getLocation();
				loc.setZ(loc.getZ() + SHIFT_OUT+16);
			}
		}  
	...
	protected void checkBlockPermissionsAndRestrictions(Player player, Block centerBlock, int regionX, int regionY, int regionZ, Location origin) throws CivException {
		...
		if (!isTownHall()) {
			...
			if (cc == null || cc.getTown() != this.town ) {
				throw new CivException("Нельзя построить здесь. Здание выходит за границы нашего Города.");
			}
		}
		...
		if (this.getConfigId().equals("s_shipyard") || this.getConfigId().equals("s_arrowship") || this.getConfigId().equals("s_scoutship") || this.getConfigId().equals("s_cannonship") || this.getConfigId().equals("ti_tradeship") || this.getConfigId().equals("w_grand_ship_ingermanland")) {
			if (!centerBlock.getBiome().equals(Biome.OCEAN) && 
				...
				!centerBlock.getBiome().equals(Biome.LUKEWARM_OCEAN) &&
				!centerBlock.getBiome().equals(Biome.SWAMP) &&
		...
		if (centertc == null && ignoreBorders == false && !isTownHall()) {
...
//		}
	...
	/*
	protected void checkChunks(Player player, Block centerBlock, int regionX, int regionY, int regionZ, Location origin) throws CivException {
		boolean autoClaim = this.autoClaim;
				...
				for (int z = 0; z < tpl.size_z; z++) {
					Block b = centerBlock.getRelative(x, y, z);
						...
						if (tpl.blocks[x][y][z].specialType == Type.COMMAND) {
							ItemManager.setTypeIdAndData(b, CivData.AIR, Bukkit.createBlockData(CivData.AIR), false);
						} else {
							ItemManager.setTypeIdAndData(b, tpl.blocks[x][y][z].getType(), tpl.blocks[x][y][z].getData(), false);
						}
		...
		class SyncTask implements Runnable {
			...
			public void run() {
				for (BlockCoord coord : structureBlocks.keySet()) {
					...
					if (ItemManager.getType(coord.getBlock()) == CivData.AIR) {
						continue;
					}
					...
					if (rand.nextInt(100) <= 70) {
						ItemManager.setType(coord.getBlock(), CivData.AIR);
						ItemManager.setData(coord.getBlock(), Bukkit.createBlockData(CivData.AIR), true);
						continue;
					}
					...
					if (rand.nextInt(100) <= 30) {
						ItemManager.setType(coord.getBlock(), CivData.GRAVEL);
						ItemManager.setData(coord.getBlock(), Bukkit.createBlockData(CivData.GRAVEL), true);
						continue;
					}
					...
					if (rand.nextInt(100) <= 10) {
						...
						ItemManager.setData(coord.getBlock(), Bukkit.createBlockData(CivData.FIRE), true);
						continue;
					}
...
//	}
	...
	public static Material getBlockIDFromSnapshotMap(HashMap<ChunkCoord, ChunkSnapshot> snapshots, int absX, int absY, int absZ, String worldName) throws CivException {
		...
		return ItemManager.getBlockType(snapshot, blockChunkX, absY, blockChunkZ);
	}
	...
	public static int getReinforcementValue(Material typeId) {
		switch (typeId) {
		case WATER:
		case LAVA:
		case AIR:
