...
import ru.tailsxcraft.civcraft.components.ActivateOnBiome;
...
public class FarmChunk {
	...
	public boolean isHydrated(Block block) {
		...
		if (beneath != null) {
			if (ItemManager.getType(beneath) == CivData.FARMLAND) {
				if (ItemManager.getData(beneath) != null)
					return true;
			}
		}
		return false;
	...
	public void spawnMelonOrPumpkin(BlockSnapshot bs, BlockCoord growMe, CivAsyncTask task) throws InterruptedException {
		...
		if (nextBlock.getType() == CivData.AIR) {
			freeBlock = nextBlock;
		}
		
		if ((nextBlock.getType() == CivData.MELON && 
				bs.getType() == CivData.MELON_STEM) ||
				(nextBlock.getType() == CivData.PUMPKIN &&
		...
		if (bs.getType() == CivData.MELON_STEM) {
			addGrowBlock("world", growMe.getX()+xOff, growMe.getY(), growMe.getZ()+zOff, CivData.MELON, Bukkit.createBlockData(CivData.MELON), true);
		} else {
			addGrowBlock("world", growMe.getX()+xOff, growMe.getY(), growMe.getZ()+zOff, CivData.PUMPKIN, Bukkit.createBlockData(CivData.PUMPKIN), true);
		}
		return;
	}
	
	public void addGrowBlock(String world, int x, int y, int z, Material melon, BlockData data, boolean spawn) {
		...
		this.growBlocks.add(new GrowBlock(world, x, y, z, melon, data, spawn));
	}
	...
	public void growBlock(BlockSnapshot bs, BlockCoord growMe, CivAsyncTask task) throws InterruptedException {
		...
		Ageable sapling = (Ageable) bs.getData();
		switch(bs.getType()) {
		case WHEAT:
		case CARROTS:
		case POTATOES:
			if (sapling.getAge() < sapling.getMaximumAge()) {
	            sapling.setAge(sapling.getAge()+1);
				addGrowBlock("world", growMe.getX(), growMe.getY(), growMe.getZ(), bs.getType(), sapling, false);
			}
			break;
		case BEETROOTS:
		case NETHER_WART:
		...
		case PUMPKIN_STEM:
			if (sapling.getAge() < sapling.getMaximumAge()) {
				sapling.setAge(sapling.getAge()+1);
				addGrowBlock("world", growMe.getX(), growMe.getY(), growMe.getZ(), bs.getType(), sapling, false);
			} else if (sapling.getAge() == sapling.getMaximumAge()) {
