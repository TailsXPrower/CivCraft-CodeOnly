...
import ru.tailsxcraft.civcraft.siege.CannonProjectile;
...
public class TownHall extends Structure implements RespawnLocationHolder {
	...
	private BlockData technameSignData; //Hold the sign's orientation
	
	private BlockCoord techdataSign;
	private BlockData techdataSignData; //Hold the sign's orientation
	...
	public BlockData getTechdataSignData() {
		return techdataSignData;
	}

	public void setTechdataSignData(BlockData techdataSignData) {
		this.techdataSignData = techdataSignData;
	}

	public BlockData getTechnameSignData() {
		return technameSignData;
	}

	public void setTechnameSignData(BlockData blockData) {
		this.technameSignData = blockData;
	}
	...
	public void createGoodieItemFrame(BlockCoord absCoord, int slotId, BlockData blockData) {
		...
		Directional wallSignData = (Directional) blockData;

		switch (wallSignData.getFacing()) {
		case EAST:
		...
		if (ItemManager.getType(itemFrameBlock) != CivData.AIR) {
			ItemManager.setType(itemFrameBlock, CivData.AIR);
		}
	...
	public void createControlPoint(BlockCoord absCoord) {
		...
		ItemManager.setType(b, CivData.FENCE); ItemManager.setData(b, Bukkit.createBlockData(CivData.FENCE));
	...
	public void onControlBlockDestroy(ControlPoint cp, World world, Player player, StructureBlock hit) {
		...
		ItemManager.setType(hit.getCoord().getLocation().getBlock(), CivData.AIR);
	...
	public void onControlBlockHit(ControlPoint cp, World world, Player player, StructureBlock hit) {
		...
		CivMessage.sendCivExceptTown(hit.getCiv(), "Один из Контрольных Блоков Города "+hit.getTown().getName()+" повреждён!", hit.getTown());
	}
	...
	public void regenControlBlocks() {
		for (BlockCoord coord : this.controlPoints.keySet()) { 
			ItemManager.setType(coord.getBlock(), CivData.OBSIDIAN);
	...
	public void onPreBuild(Location loc) throws CivException {		
		...
		if (oldTownHall != null) {
			...
			if (cc == null || cc.getTown().getCiv() != this.getTown().getCiv()) {
				throw new CivException("Вы не можете перестроить Ратушу за пределами границ Цивилизации.");
			}
			...
			try {
				for ( BlockCoord bc : this.getTown().getTownHall().getStructureBlocks().keySet()) {
					TownChunk.unclaim(CivGlobal.getTownChunk(bc.getLocation()));
				}
