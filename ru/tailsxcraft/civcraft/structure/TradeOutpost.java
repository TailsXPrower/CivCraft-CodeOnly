...
public class TradeOutpost extends Structure {
	...
	public void build_trade_outpost_tower(BlockFace blockFace) throws CivException {
		/* Add trade good to town. */
		...
		for (int i = 0; i < 3; i++) {
			...
			ItemManager.setType(b, CivData.BEDROCK); ItemManager.setData(b, b.getBlockData());
		...
		switch(blockFace) {
		case WEST: 
			b = centerLoc.getBlock().getRelative(-1, 2, 0);
			break;
		case EAST: 
			b = centerLoc.getBlock().getRelative(1, 2, 0);
			break;
		case SOUTH: 
			b = centerLoc.getBlock().getRelative(0, 2, 1);
			break;
		case NORTH: 
			b = centerLoc.getBlock().getRelative(0, 2, -1);
			break;
		...
		Directional direction = (Directional) Bukkit.createBlockData(CivData.WALL_SIGN);
		direction.setFacing(blockFace);
		ItemManager.setType(b, CivData.WALL_SIGN); 
		ItemManager.setData(b, direction);
		...
		case EAST: 
			...
			break;
		case SOUTH: 
			b = centerLoc.getBlock().getRelative(0, 1, 1);
			break;
		case NORTH: 
			b = centerLoc.getBlock().getRelative(0, 1, -1);
			break;
		...
		if (entity == null || (!(entity instanceof ItemFrame))) {
			this.frameStore = new ItemFrameStorage(b.getLocation(), blockFace);	
		} else {
	...
	public void fancyDestroyStructureBlocks() {
		for (BlockCoord coord : this.structureBlocks.keySet()) {
			...
			if (ItemManager.getType(coord.getBlock()) == CivData.BEDROCK || ItemManager.getType(coord.getBlock()) == CivData.AIR) {
