...
import ru.tailsxcraft.civcraft.threading.tasks.NotificationTask;
...
public class Blacksmith extends Structure {
	...
	public void saveItem(ItemStack item, String key) {
		...
		String value = ""+ItemManager.getType(item)+":";
		
		for (Enchantment e : item.getEnchantments().keySet()) {
			value += ItemManager.getType(e)+","+item.getEnchantmentLevel(e);
			value += ":";
		}
	...
	public static boolean canSmelt(Material blockid) {
		switch (blockid) {
		case GOLD_ORE:
		case IRON_ORE:
			return true;
	 ...
	 */
	public static Material convertType(Material blockid) {
		switch(blockid) {
		case GOLD_ORE:
		...
		if (freeStr == null) {
			...
			for (int i = 0; i < lore.length; i++) {
				...
				if (str.contains("free enhancements")) {
					if (level != 0) {
						lore[i].replaceAll(str, CivColor.LightBlue+CivSettings.localize.localizedString("var_blacksmith_forge_loreFreeEnchancements",level));
					} else {
						lore[i].replaceAll(str, "");
					}
					break;
				}
			}
	...
	public void withdrawSmelt(Player player) throws CivException {
		...
		for (SessionEntry se : entries) {
			...
			Material itemId = Material.IRON_INGOT;
