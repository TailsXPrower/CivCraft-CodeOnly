...
public class MultiInventory {
	...
	private boolean isCorrectItemStack(ItemStack stack, String mid, Material material, short data) {
		if (stack == null) {
			return false;
		}
		...
		if (mid != null) {
			...
			if (ItemManager.getType(stack) != material) {
				return false;
			}
	 ...
	 */
	public boolean removeItem(String mid, Material material, short data, int amount, Boolean direct) throws CivException {
		ArrayList<ItemInvPair> toBeRemoved = new ArrayList<ItemInvPair>();
		
		int count = amount;
		for (Inventory inv : invs) {
			for (ItemStack item : inv.getContents()) {
				if(!isCorrectItemStack(item, mid, material, data)) {
					continue;
				}
				...
				if (item.getAmount() > count) {
					toBeRemoved.add(new ItemInvPair(inv, mid, material, data, count));
					count = 0;
					break;
					...
					toBeRemoved.add(new ItemInvPair(inv, mid, material, data, item.getAmount()));
	...
	public boolean removeItem(ItemStack item, Boolean direct) throws CivException {
		...
		if (loreMat != null) {
			return removeItem(loreMat.getId(), null, (short)0, item.getAmount(), direct);
		} else {
			/* Vanilla item. no custom id. */
			return removeItem(null, ItemManager.getType(item), ItemManager.getData(item), item.getAmount(), direct);
		}
	}
		...
		for (Inventory inv : invs) {
			for (ItemStack item : inv.getContents()) {		
				...
				if (mid != null) {
					...
					/* Vanilla item. */
					if (ItemManager.getType(item) != type) {
						continue;
					}
