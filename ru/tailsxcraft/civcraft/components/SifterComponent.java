...
public class SifterComponent extends Component {
	...
	public void addSiftItem(Material material, short source_data, double rate, Material material2, short result_data, int amount) {
		SifterItem si = new SifterItem();
		si.source_type = material;
		si.source_data = source_data;
		si.rate = rate*1000;
		si.result_type = material2;
	...
	private void process(MultiInventory source, MultiInventory dest, int count, CivAsyncTask task) {
		...
		for (ItemStack stack : source.getContents()) {
			if (stack == null || ItemManager.getType(stack) == null) {
				continue;
			}
			...
			for (SifterItem si : items) {
				...
				if (si.source_type != ItemManager.getType(stack)) {
					continue;
				}
			...
			try {
				task.updateInventory(Action.ADD, dest, ItemManager.createItemStack(lowestChanceItem.result_type, lowestChanceItem.amount));
			} catch (InterruptedException e) {
				return;
			}
