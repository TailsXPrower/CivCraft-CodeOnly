...
import ru.tailsxcraft.civcraft.config.ConfigTempleLevel;
...
//	private Buildable buildable;
	 ...
	 */
	private HashMap<Integer, Map<Material, Integer>> consumptions = new HashMap<Integer, Map<Material, Integer>>();
	
	/*
	 * Contains a list of equivilent exchanges 
	 */
	private HashMap<Material, ConsumeLevelEquivExchange> exchanges = new HashMap<Material, ConsumeLevelEquivExchange>();
...
//	consumeComp.createComponent(this);
	...
	public void createComponent(Buildable buildable, boolean async) {
		...
		if (buildable instanceof Temple) {
			...
			for (ConfigMineLevel lvl : CivSettings.mineLevels.values()) {
				...
				HashMap<Material, Integer> redstoneAmounts = new HashMap<Material, Integer>();
				redstoneAmounts.put(CivData.REDSTONE_DUST, lvl.amount);
				this.setConsumes(lvl.level, redstoneAmounts);
			}
		} else if (buildable instanceof University) {
			for (ConfigUniversityLevel lvl : CivSettings.universityLevels.values()) {
				this.addLevel(lvl.level, lvl.count);
				
				HashMap<Material, Integer> booksAmounts = new HashMap<Material, Integer>();
				booksAmounts.put(Material.BOOK, lvl.amount);
				this.setConsumes(lvl.level, booksAmounts);
			}
		}
	...
	public void addEquivExchange(Material bread, Material fishRaw, int basePerAlt) {
		ConsumeLevelEquivExchange ee = new ConsumeLevelEquivExchange();
		ee.baseType = bread;
		ee.altType = fishRaw;
		ee.basePerAlt = basePerAlt;		
		this.exchanges.put(bread, ee);
	}
	...
	private boolean hasEnoughToConsume() {
		...
		foundCounts = new HashMap<Material, Integer>();
		for (ItemStack stack : source.getContents()) {
			if (stack == null) {
				continue;
			}
			
			boolean isRequire = thisLevelConsumptions.containsKey(ItemManager.getType(stack));
			boolean isEquiv = false;
			
			for (ConsumeLevelEquivExchange ee : this.exchanges.values()) {
				if (ee.altType == ItemManager.getType(stack)) {
					isEquiv = true;
					break;
				}
			}
		//	CivLog.debug("type:"+stack.getTypeId()+" equiv:"+isEquiv);
			...
			Integer count = foundCounts.get(ItemManager.getType(stack));
			if (count == null) {
				count = stack.getAmount();
			} else {
				count += stack.getAmount();
			}
			foundCounts.put(ItemManager.getType(stack), count);
		}
