...
import ru.tailsxcraft.civcraft.util.NBTStaticHelper;
...
public class AttributeUtil {
   ...
   // private List<String> lore = new LinkedList<String>();
    ...
    public net.minecraft.server.v1_16_R3.ItemStack nmsStack;
    ...
    public void addLore(String str) {
    	...
    	ItemStack modifiedItemStack = CraftItemStack.asCraftMirror(nmsStack);
		ItemMeta modifiedMeta = modifiedItemStack.getItemMeta();
		if ( modifiedMeta == null ) {
			return;
		}
		List<String> lore = modifiedMeta.getLore();
		if ( lore == null ) {
		...
		modifiedItemStack.setItemMeta(modifiedMeta);
		
		this.nmsStack = CraftItemStack.asNMSCopy(modifiedItemStack);  	 
    }
    ...
    public void setLore(String[] strings) {
		...
		modifiedMeta.setLore(Arrays.asList(strings));
    ...
    public void addEnhancement(String enhancementName, String key, String value) {
    	...
    	ItemStack ItemStack = CraftItemStack.asCraftMirror(nmsStack);
    	if ( ItemStack == null || ItemStack.getType() == Material.AIR) return;
    	NBTItem nbti = new NBTItem(ItemStack);
		
    	NBTCompound enhanments = nbti.getOrCreateCompound("item_enhancements");
    	NBTCompound enhs = enhanments.getOrCreateCompound(enhancementName);
    	if (key != null) {
    		_setEnhancementData(enhs, key, value);
    	}
    	enhs.setString("name", enhancementName);
		
		ItemStack modifiedItemStack = nbti.getItem();
...
//	
    ...
    private void _setEnhancementData(NBTCompound enhCompound, String key, String value) {
    	if (key.equalsIgnoreCase("name")) {
    		throw new IllegalArgumentException();
    	}
    	
    	enhCompound.setString(key, value);
    }
	...
	public String getEnhancementData(String enhName, String key) {
		...
		ItemStack modifiedItemStack = CraftItemStack.asCraftMirror(nmsStack);
		NBTItem nbti = new NBTItem(modifiedItemStack);
		
		if (!nbti.getCompound("item_enhancements").getCompound(enhName).hasKey(key)) {
			return null;
		}
				
		return nbti.getCompound("item_enhancements").getCompound(enhName).getString(key);
	}
	...
	public LinkedList<LoreEnhancement> getEnhancements() {
		...
		ItemStack modifiedItemStack = CraftItemStack.asCraftMirror(nmsStack);
		NBTItem nbti = new NBTItem(modifiedItemStack);
    	NBTCompound compound = nbti.getCompound("item_enhancements");
    	
    	if ( compound == null ) {
    		return returnList;
    	}

    	for (Object keyObj : compound.getKeys()) {
    		if (!(keyObj instanceof String)) {
    			continue;
    		}
    		
    		String key = (String)keyObj;
    		Object obj = compound.getCompound(key);
    		
    		if (obj instanceof NBTCompound) {
    			NBTCompound enhCompound = (NBTCompound)obj;
    ...
    public boolean hasEnhancement(String enhName) {
		...
		if (nbti.getCompound("item_enhancements") == null) {
			return false;
		}
				
		return nbti.getCompound("item_enhancements").hasKey(enhName);
	}
	...
	public boolean hasEnhancements() {
		...
		return nbti.hasKey("item_enhancements");
	}
    ...
    public void setCivCraftProperty(String key, String value) {
    	...
    	civcraftCompound.set(key, NBTTagString.a(value));
	...
	public void setName(String name) {
		...
		modifiedMeta.setDisplayName(CivColor.BOLD+CivColor.White+name);
	...
	public String getName() {
		...
		return modifiedMeta.getDisplayName();
	}
	...
	public void setColor(Long long1) {
		...
		displayCompound.set("color", NBTTagInt.a(long1.intValue()));
	...
	public void setSkullOwner(String string) {
		...
		skullCompound.set("Name", NBTTagString.a(string));
